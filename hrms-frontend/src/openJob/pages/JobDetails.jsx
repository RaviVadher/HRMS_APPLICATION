...
import { updateJobStatus } from "../jobAPI";
...
export default function JobDetails()
{
    ...
    const[status,setStatus] = useState("Open")
        ...
        if(reload || loading) return<DashboardLayout><CircularProgress/></DashboardLayout>
         ...
         <span className="text "><b>Status: </b>{job.status}</span>
          ...
          {user.role ==="ROLE_Hr" && job.status==="Open" &&( 
           ...
           {user.role==="ROLE_Hr" && (
            ...
            {tab===0? <ReferTable jobId={job.jobId} /> : <ShareTable jobId={job.jobId}/> }
            </>
             )}

    <ShareForm open={shareOpen} close={()=>setShareOpen(false)} job={job}/>
    <ReferForm open={referOpen} close={()=>setReferOpen(false)} job={job}/>
   </div>
   </DashboardLayout>
